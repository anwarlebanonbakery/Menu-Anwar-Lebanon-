...
export async function fetchMenuData() {
  try {
    ...
    const heroFallbackPromise = getDoc(doc(db, "categories", "__hero_settings__")).catch(() => null);
    ...
    const categories = catsSnap.docs
      .filter(d => d.id !== "__hero_settings__")
...
export async function saveHeroSettings(settings) {
  ...
  try {
    ...
    // لكنها لا تحتوي Rule لمسار siteSettings. نستخدم مستندًا محجوزًا داخل
    // categories كـ fallback حتى لا يتعطل حفظ الـ Hero.
    try {
      await setDoc(doc(db, "categories", "__hero_settings__"), {
        ...clean,
