...
export async function fetchMenuData() {
  try {
    ...
    const offersQ = liveState.isAdmin
      ? query(collection(db, "offers"), orderBy("displayOrder"))
      : query(collection(db, "offers"), where("visible", "==", true), orderBy("displayOrder"));

    const [catsSnap, prodsSnap, offersSnap] = await Promise.all([getDocs(catsQ), getDocs(prodsQ), getDocs(offersQ)]);
...
export async function updateOffer(id, data) {
  return updateDoc(doc(db, "offers", id), { ...data, updatedAt: serverTimestamp() });
}
...
export async function reorderOffers(orderedIds) {
  const batch = writeBatch(db);
  orderedIds.forEach((id, i) => {
    batch.update(doc(db, "offers", id), { displayOrder: i, updatedAt: serverTimestamp() });
  });
  return batch.commit();
}
