...
export async function fetchMenuData() {
  try {
    ...
    const [catsSnap, prodsSnap, heroSnap, heroFallbackSnap] = await Promise.all([
      getDocs(catsQ),
      getDocs(prodsQ),
      heroPromise,
      heroFallbackPromise
...
export async function reorderOffers(orderedIds) {
  const current = Array.isArray(liveState.offers) ? liveState.offers : [];
  const hasEmbedded = current.some(o => o?._storage === 'embedded');
  if (hasEmbedded) {
    ...
    (Array.isArray(orderedIds) ? orderedIds : []).forEach(id => {
      ...
      seen.add(key); ordered.push({ ...byId.get(key), _storage: 'embedded' });
    });
    current.forEach(o => { const key = String(o.id ?? '').trim(); if (key && !seen.has(key)) { seen.add(key); ordered.push({ ...o, _storage: 'embedded' }); } });
    const normalized = normalizeEmbeddedOffers(ordered);
    await saveEmbeddedOffers(normalized);
    liveState.offers = normalized;
    return true;
  }
  ...
  (Array.isArray(orderedIds) ? orderedIds : []).forEach(id => {
    const key = String(id ?? '').trim();
    if (!key || seen.has(key) || !byId.has(key)) return;
    seen.add(key); ordered.push(byId.get(key));
  });
  current.forEach(o => { const key = String(o.id ?? '').trim(); if (key && !seen.has(key)) { seen.add(key); ordered.push(o); } });
  const batch = writeBatch(db);
  ordered.forEach((o, i) => batch.update(doc(db, 'offers', String(o.id)), { displayOrder: i, updatedAt: serverTimestamp() }));
  await batch.commit();
  return true;
}
