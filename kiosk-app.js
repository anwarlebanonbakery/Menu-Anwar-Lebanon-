...
} from "./firebase-app.js?v=20260902-37";
...
function renderHomeGrid() {
  ...
  grid.innerHTML = liveState.categories.map(cat => {
    ...
    const onclick = cat.displayType === 'image'
      ? `safeInvoke(openImageMenu,decodeURIComponent('${encodedMenuUrl}'),decodeURIComponent('${encodedName}'))`
...
function buildProductCard(item) {
  ...
  card.onclick = () => safeInvoke(openImageMenu, item.imageUrl, item.name);
