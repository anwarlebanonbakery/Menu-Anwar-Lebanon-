...
} from "./firebase-app.js?v=20260902-36";
...
function renderHero() {
  ...
  if(bg){
    ...
    const fit = ['cover','contain','fill'].includes(c.imageFit) ? c.imageFit : 'cover';
    const fitCss = fit === 'fill' ? '100% 100%' : fit;
    bg.style.backgroundPosition = `${Number(c.backgroundPositionX ?? 50)}% ${Number(c.backgroundPositionY ?? 0)}%`;
    bg.style.backgroundRepeat = 'no-repeat';
    bg.style.setProperty('--hero-image-fit', fitCss, 'important');
    bg.style.setProperty('background-size', fitCss, 'important');
    bg.classList.remove('hero-fit-cover','hero-fit-contain','hero-fit-fill');
    bg.classList.add(`hero-fit-${fit}`);
    const zoomEnabled = anim.imageZoom === true && fit === 'cover';
