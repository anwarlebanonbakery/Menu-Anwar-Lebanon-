...
} from "./firebase-app.js?v=20260902-6";
...
function heroOverlayStyle(o) {
  ...
  return `linear-gradient(${o.direction||'to-bottom'}, ${transparent} 18%, ${color} 100%)`;
}
function renderHero() {
  ...
  hero.style.setProperty('--hero-h-desktop', `${Number(c.heightDesktop||500)}px`);
...
function teardownOffersShowcase() {
  // تم تحويل عروضنا إلى نفس تجربة معرض الصور؛ لا توجد timers خاصة بالعروض الآن.
}
    ...
    if (isOffers && !item.photoOnly) {
      ...
      const details = (title || price !== '') ? `
        <div class="offer-gallery-details">
          ...
          ${(price !== '' || old !== '') ? `<div class="offer-gallery-price">
